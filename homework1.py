...
# *** Дополнительная задача к первой задаче. Напишите программу, которая принимает на вход 2 числа: номер месяца и номер дня в месяце, проверяет является ли день выходным.
...
# Задача 2.*** Со звездочкой. Напишите программу для проверки истинности утверждения ¬(X ⋁ Y ⋁ Z) = ¬X ⋀ ¬Y ⋀ ¬Z для всех значений предикат.
# Не решено.
...
# при каком х и y будет та или иная плоскость:
# x>0 и y>0 = 1
# y>0 и x<0 = 2
# x<0 и y<0 = 3
# x>0 и y<0 = 4
...
# Как найти расстояние?: |AB| = корень из ((x2 - x1) во 2 степени + (y2 - y1) во 2 степени)
